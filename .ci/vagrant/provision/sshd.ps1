...
if ((Test-Path -Path "C:\code\.ci\vagrant\provision\files\authorized_keys" )) {
    Write-Output "Copying C:\code\.ci\vagrant\provision\files\authorized_keys to ~\.ssh\authorized_keys"
    $src = "/cygdrive/c/code/.ci/vagrant/provision/files/authorized_keys"
    $dst = "/home/vagrant/.ssh/authorized_keys"
    Run $BASH "--login -c 'cp $src $dst"
    Run $BASH "--login -c 'chmod 644 $dst"
} else {
    Write-Output "C:\code\.ci\vagrant\provision\files\authorized_keys does not exist"
}

# Replace the sshd configuration with our own
Write-Output "Replace SSHD configuration"
$src = "/cygdrive/c/code/.ci/vagrant/provision/files/sshd_config"
$dst = "/etc/sshd_config"
Run $BASH "--login -c 'cp $src $dst"
